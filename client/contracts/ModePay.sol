...
contract ModePay {
    ...
    modifier notPaid(
        ...
        require(
            !splits[groupId][splitId].hasPaid[participant],
    ...
    function transferToAddress(
        ...
        require(
            msg.value == amount,
            "Amount sent must match the specified amount"
        );
        recipient.transfer(amount);
        emit FundsTransferred(msg.sender, recipient, amount);
        uint256 reward = getRandomNumber();
        address payable sender = payable(msg.sender);
        if (address(this).balance > reward) sender.transfer(reward);
        else reward = 0;
    ...
    function makePayment(
        ...
        external
        payable
        splitExists(groupId, splitId)
        notPaid(groupId, splitId, msg.sender)
    {
        require(
            msg.value == splits[groupId][splitId].perShare,
            "Incorrect payment amount"
        );

        require(
            splits[groupId][splitId].hasPaid[msg.sender] == false,
            "Already paid"
        );

        transferToAddress(payable(splits[groupId][splitId].owner), msg.value);

        splits[groupId][splitId].amountLeft =
            splits[groupId][splitId].amountLeft -
            msg.value;
    ...
    function getParticipants(
        uint256 groupId,
        uint256 splitId
    ) external view splitExists(groupId, splitId) returns (address[] memory) {
    ...
    function getAmount(
        uint256 groupId,
        uint256 splitId
    ) external view splitExists(groupId, splitId) returns (uint256) {
    ...
    function getReason(
        uint256 groupId,
        uint256 splitId
    ) external view splitExists(groupId, splitId) returns (string memory) {
    ...
    function hasPaid(
        ...
        uint256 splitId,
        address participant
    ) external view splitExists(groupId, splitId) returns (bool) {
    ...
    function getSplitOwner(
        uint256 groupId,
        uint256 splitId
    ) external view splitExists(groupId, splitId) returns (address) {
